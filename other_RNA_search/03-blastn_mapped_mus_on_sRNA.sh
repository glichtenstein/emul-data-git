...
# DataBase
DATABASE="$WORKDIR/02-reference_seqs/tRNAs/mus_musculus_db.fasta";
...
for i in *.mus.fa; do arrfasta+=( "$i" );done
...
do
 blastn \
	...
	-evalue "0.01" \
	-num_threads "8";
	rename 's/.mus.fa.blastn/.mus.blastn/' "$OUTPUT/${arrfasta[ $count ]}.blastn";
	(( count++ ));
done
########################################################################
